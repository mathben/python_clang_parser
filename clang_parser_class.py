...
class ASTObject(object):
    def __init__(self, cursor, filename, _clang_parser):
        ...
        self.count_variable = _clang_parser.get_variables_statistic(cursor)
        self.keywords = None
...
class Class(ASTObject):
    def __init__(self, cursor, filename, _clang_parser):
        super(Class, self).__init__(cursor, filename, _clang_parser)

        self.methods = [Method(c_child, filename, _clang_parser) for c_child in cursor.get_children() if
...
class Function(ASTObject):
    def __init__(self, cursor, filename, _clang_parser):
        super(Function, self).__init__(cursor, filename, _clang_parser)
        self.keywords = _clang_parser.get_tokens_statistic(cursor) + collections.Counter(var=self.count_variable)
...
class ClangParser(object):
    def __init__(self, parser):
        # instance for clang_parser()
        # second arg, True to display diagnostic
        self._parser = parser
        _clang_lib = clang.cindex.conf.lib
        self._index = clang.cindex.Index(_clang_lib.clang_createIndex(False, parser.show_missing_header_file))
    ...
    def build_classes(self, cursor, filename, dir_name, is_first_call=True):
        result = []
        # ignore Cursor Kind
        ...
        # TODO function flip of /home/mathben/poly/LOG6302-réingénérie/opencv/modules/cudaarithm/src/core.cpp not work
        ...
        if is_first_call:
            ...
            result = self.merge_method(result)
    ...
    def clang_parser(self, arg):
        _file_name = arg[0]
        _dir_name = arg[1]
        _clang_arg = arg[2]

        absolute_file_path = os.path.join(_dir_name, _file_name)
        options = clang.cindex.TranslationUnit.PARSE_SKIP_FUNCTION_BODIES if self._parser.remove_token else 0
        _translation_unit = self._index.parse(absolute_file_path, ['-x', 'c++', '-std=c++11'] + _clang_arg,
                                              options=options)
        _result = self.build_classes(_translation_unit.cursor, _file_name, _dir_name)
